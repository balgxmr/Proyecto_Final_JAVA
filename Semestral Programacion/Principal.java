...
public class Principal {
  public static void main(String[] args)
    ...
    while (true) {
      ...
      if (turnoJuego) { // verificar tablero e indentificar el ganador
        ...
        if (!jugar(jugador1, jugador2.getTablero())) { // verificar si el
                                                       // jugador se ha rendido
          turnoJuego = !turnoJuego;
          break;
        }
        if (jugador2.getTablero()
                .recorrerTablero()) { // llamar funcion para recorer el tablero
                                      // y verificar la condicion de los barcos
          break;
        }

      } else {

        if (!jugar(jugador2, jugador1.getTablero())) { // verificar si el
                                                       // jugador se ha rendido
          turnoJuego = !turnoJuego;
          break;
        }
        if (jugador1.getTablero()
                .recorrerTablero()) { // llamar funcion para recorer el tablero
                                      // y verificar la condicion de los barcos
          break;
        }
  ...
  public static void inicializarJuego(Jugador jugador) { // inicio del juego
    ...
    for (int i = 0; i <= 3; i++) {
      while (true) { // selecion de el tipo de barco
        ...
        if (jugador.getBarco(optionBarco - 1)
                .getSeleccionado()) { // en caso de barco ya seleccionado
    ...
    while (true) {
      ...
      if (seleccionar == 2) {
        System.out.println(
            "Cual barco desea cambiar de posicion?\n"); // selecion de el tipo
                                                        // de barco a cambiar
        optionBarco = datos.capturarOpcionBarco(
  ...
  public static void inicializarBarco(Jugador jugador, int numeroBarco) {
    ...
    while (true) {
      ...
      // selecionar posicion del barco si es horizontal o vertica
      {
        ...
        if (jugador.getTablero().validarEspacio(
                filaIndex, columnaIndex,
                jugador.getBarco(numeroBarco - 1).getTamaño(), optionForma))
        // marcar como seleccionado el barco escojido por el jugador si estan
        // los espacios disponibles
        {
          jugador.getTablero().setBarco(
              filaIndex, columnaIndex, numeroBarco,
              jugador.getBarco(numeroBarco - 1).getTamaño(), optionForma);
          ...
          System.out.println(
              "No hay suficiente espacio para colocar el barco, intentelo de nuevo");
        }
  ...
  public static boolean jugar(Jugador jugador, Tablero tableroOponente)
    ...
    while (true) {
      ...
      if (coordenadas.equals(
              "EXIT")) { // si coordenadas es igual a Exit termina el juego
        return false;
      }
      fila = Character.toUpperCase(
          coordenadas.charAt(0)); // Dar el caracter de la posicion 0 a fila
      columna = Integer.parseInt(
          coordenadas.substring(1)); // convertir la subcadela de la posicion 1
                                     // a entero y asignar a columna
      filaIndex = (int)fila - 'A'; // converit el char asignado a fila a un
                                   // numero y asignar a filaIndex
      columnaIndex = columna - 1; // asignar el valor de columna a columnaIndex

      if (jugador.getTablero().validarCoordenada(filaIndex, columnaIndex) &&
          tableroOponente.atacarCoordenada(
              filaIndex, columnaIndex)) { // validar si la coordenada es valida
                                          // y si a dado en el blanco o no
        break;

      } else { // error de coordenadas incorrectas
        System.out.println("\u001B[31m Vuelve a introducir las coordenadas"
                           + "\u001B[0m");
      }
    }
    return true;
  ...
  public static void
  finalizarJuego(Jugador jugador1, Jugador jugador2,
                 boolean turnoJuego) { // impresiones de fin del juego
    ...
    System.out.println("\nBarcos de " + jugador1.getName() +
                       ":"); // impresion de los barcos del jugador N°1
    for (int i = 1; i <= 4; i++) {
      System.out.println("Barco #" + i + ": " +
                         jugador1.getTablero().capturarPosicionBarco(i));
    }
    System.out.println("\nBarcos de " + jugador2.getName() +
                       ":"); // impresion de los barcos del jugador N°2
    for (int i = 1; i <= 4; i++) {
      System.out.println("Barco #" + i + ": " +
                         jugador2.getTablero().capturarPosicionBarco(i));
    }
  }
}
