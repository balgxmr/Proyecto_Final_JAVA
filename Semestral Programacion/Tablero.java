...
public class Tablero {
  ...
  public Tablero() { this.tablero = inicializarMatriz(); }
  ...
  public void setBarco(int fila, int columna, int numeroBarco, int tamaño,
                       int optionForma)
    ...
    for (int i = 0; i < tablero.length; i++) {
      ...
      for (int j = 0; j < tablero.length; j++) {
        if (tablero[i][j] >= '1' && tablero[i][j] <= '4') {
          if (ocultarBarco) { // ocultar posicion de los barcos mientras se
                              // juega
            System.out.print("-   ");
  ...
  public void
  eliminarBarco(int numeroBarco) { // Eliminar barco para cambiar de porsicion
    char letraBarco = Integer.toString(numeroBarco).charAt(0);
    for (int i = 0; i < tablero.length; i++) {
      for (int j = 0; j < tablero.length; j++) {
  ...
  public boolean atacarCoordenada(int fila, int columna)
      throws InterruptedException { // atacar possicion en el tablero enemigo
    int valor;
    char valorConvertido;
    if (tablero[fila][columna] == 'X' ||
        (tablero[fila][columna] >= 'A' && tablero[fila][columna] <= 'D')) {
      System.out.println("\n\u001B[31m Ya atacó este lugar, intentelo de nuevo."
                         + "\u001B[0m");
      return false;
    }
  ...
  public boolean recorrerTablero() { // recorremos en tablero para saber la
                                     // condicion de los barcos
    for (int i = 0; i < tablero.length; i++) {
      for (int j = 0; j < tablero.length; j++) {
        if (tablero[i][j] >= '1' && tablero[i][j] <= '4') {
          return false;
  ...
  public String capturarPosicionBarco(
      int numeroBarco) { // Captura y valida las coordenadas de los barcos
    ...
    for (int i = 0; i < tablero.length; i++) {
      for (int j = 0; j < tablero.length; j++) {
        if (tablero[i][j] == letra ||
            tablero[i][j] == numero) { // validad si las coordenadas estan en el
                                       // formato correcto
          coordenadaFila = (char)('A' + i);
          coordenadaColumna = j + 1;

          if (i < tablero.length - 1 &&
              (tablero[i + 1][j] == letra || tablero[i + 1][j] == numero)) {
            coordenadaFinal = coordenadaFila + "" + coordenadaColumna +
                              " Vertical hacia abajo";
            return coordenadaFinal;
          } else if (j < tablero.length - 1 && (tablero[i][j + 1] == letra ||
                                                tablero[i][j + 1] == numero)) {
            coordenadaFinal = coordenadaFila + "" + coordenadaColumna +
